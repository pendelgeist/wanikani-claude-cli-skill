...
export async function lessonsCommand(client, { start = false } = {}) {
  ...
  const subjects = await client.getSubjectsByIds(assignments.map((a) => a.data.subject_id));
  ...
  console.log(`${items.length} lesson${items.length === 1 ? "" : "s"} available.`);
