...
import { isMeaningCorrect, readingVerdict, splitAnswer } from "../grading.js";
 ...
 * miss. `initial` is a half already typed at the combined prompt — it's
 * graded before anything else is asked for.
 */
async function quizPart({ ask, label, judge, answer, initial = null }) {
  let wrong = 0;
  let pending = initial;
  for (;;) {
    const input = pending === null ? await ask(`  ${label}> `) : pending;
    pending = null;
...
export async function reviewCommand(client, { limit, input = stdin, output = stdout } = {}) {
  ...
  console.log(
    `${queue.length} review${queue.length === 1 ? "" : "s"} due. ` +
      `Meaning and reading can go on one line — "fur, ke". ` +
  ...
  try {
    for (const item of queue) {
      ...
      // One prompt for both halves, split by the same code the chat skill
      // grades with — so "fur, ke" means the same thing in either place.
      let opening = { meaning: null, reading: null };
      if (item.needsReading) {
        const reply = await ask("  answer> ");
        if (reply === null || reply.trim() === ":quit") break;
        opening =
          reply.trim() === ":show"
            ? { meaning: ":show", reading: ":show" }
            : splitAnswer(reply, item.subject, true);
      }
