...
import { loadSitting, loadQueueOrder, loadPriorGrades, openItems, resumableSitting } from "../queueOrder.js";
...
async function sittingState() {
  ...
  return {
    sitting: true,
    stale: !live,
    // Aged out of the list but not out of the scoreboard — the two have
    // different lives, and `status` is where someone goes to find out which.
    resumable: !live && (await resumableSitting()) !== null,
...
function nextCall(state) {
  if (state.stale) {
    return state.resumable
      ? "`ask` fetches a fresh list and carries on this sitting's totals."
      : "`ask` starts a fresh sitting.";
  }
...
export function statusReport(state) {
  ...
  if (state.stale) {
    lines.push(
      `Sitting: idle ${age}, past its 30-minute life. The next \`ask\` fetches a fresh list and ` +
        ...
        (state.resumable ? " The running totals below carry over: a break isn't a new sitting." : ""),
    );
  } else {
  ...
  if (state.sessionSubmitted > 0) {
    const when = state.stale && !state.resumable ? "Sent before it went idle" : "Sent this sitting";
