 ...
 * two counts are this attempt's contribution to `submit-batch`. The caller
 * keeps the judgment call it's actually good at, and overrides.
 */
export async function gradeCommand(client, { subjectId, answer, meaning, reading }) {
  ...
  const verdict = { subjectId, parsed: given, meaning: meaningStatus, reading: readingStatus };
  ...
  if (needsReading && !given.reading) {
    return print({ ...verdict, wrongMeaning: 0, wrongReading: 0, say: ASK_READING, open: true });
  }

  if (readingStatus === "incorrect") {
    return print({ ...verdict, wrongMeaning: 0, wrongReading: 1, say: corrections.reading, open: false });
  }

  return print({ ...verdict, wrongMeaning: 0, wrongReading: 0, say: null, open: false });
}
