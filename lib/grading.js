...
/**
 * Punctuation at either end of a reading is typing, not sound — and unlike a
 * meaning, which has been trimmed this way since the start, a reading carried
 * it all the way into kana conversion, where wanakana turns "." into 。. So
 * `.sei` became 。せい and matched nothing: 青 answered `.sei` and 間 answered
 * `.kan` were both marked wrong in one sitting for readings the user had
 * right, because leading a reply with a full stop is a typing habit and not a
 * different answer.
 *
 * `-` is deliberately not in the set: it is how ー gets typed ("pe-ji"), so
 * stripping it from the end would break the reading it exists to spell.
 */
const trimReadingPunctuation = (text) => (text ?? "").replace(TRIM_PUNCTUATION, "");
...
export function normalizeReadingInput(text) {
  return wanakana.toKana(normalizeAmbiguousRomaji(trimReadingPunctuation(text)));
}
...
export function readingCandidates(input) {
  const text = normalizeAmbiguousRomaji(trimReadingPunctuation(input)).replace(STRAY_APOSTROPHE, "");
 ...
 */
const acceptedMeaningEntries = (subject) =>
  [
    ...subject.meanings.filter((m) => m.accepted_answer).map((m) => m.meaning),
    ...(subject.auxiliary_meanings || [])
      .filter((m) => m.type === "whitelist")
      .map((m) => m.meaning),
    // WaniKani's own casing is kept alongside the normalized form: the
    // near-miss line names the meaning an answer was close to, and it has to
    // name it the way the correction line above it does.
  ].map((meaning) => ({ meaning, normalized: normalizeMeaning(meaning) }));

const acceptedMeanings = (subject) => acceptedMeaningEntries(subject).map((entry) => entry.normalized);

const blacklistedMeanings = (subject) =>
  (subject.auxiliary_meanings || [])
    .filter((m) => m.type === "blacklist")
    .map((m) => normalizeMeaning(m.meaning));

/**
 * Wrong, but wrong by a hair — one edit further out than the tolerance above.
 *
 * 酉 was answered "alcholol" and marked wrong: "alcohol" is seven letters, so
 * it forgives one slip and that was two. The table is right — loosening it
 * starts accepting answers nobody knew — but `--forgive` exists for exactly
 * this case and had not been used once in six sittings, because it lived in
 * the skill file rather than on the screen. So the CLI says when it's close,
 * the same way it says the lookup link: a feature nobody can see isn't one.
 *
 * A blacklisted meaning is never near: those are the ones WaniKani lists to
 * say "looks right, isn't", and inviting an override on one would be teaching
 * the wrong answer. This only ever *offers*; the verdict on the record stays
 * wrong until somebody says otherwise.
 *
 * It hands back *which* meaning was nearly reached, rather than a yes, because
 * the judgment the offer asks for is a comparison and the comparison was being
 * made on a feeling. 伝える answered "to transfer" is three edits from "To
 * Transmit" — near enough to be flagged, and a different word — and it was
 * forgiven on the strength of the flag alone. The words are the evidence, so
 * the line carries them.
 */
export function nearMissMeaning(input, subject) {
  const normalizedInput = normalizeMeaning(input);
  if (!normalizedInput || isMeaningCorrect(input, subject)) return null;
  if (blacklistedMeanings(subject).includes(normalizedInput)) return null;

  let best = null;
  for (const entry of acceptedMeaningEntries(subject)) {
    const distance = editDistance(normalizedInput, entry.normalized);
    if (distance > typoTolerance(entry.normalized) + 1) continue;
    if (best === null || distance < best.distance) best = { meaning: entry.meaning, distance };
  }
  return best?.meaning ?? null;
}

export function isNearMiss(input, subject) {
  return nearMissMeaning(input, subject) !== null;
}
...
export function isMeaningCorrect(input, subject) {
  ...
  const accepted = acceptedMeanings(subject);
...
const looksLikeReading = (text, subject) =>
  readingVerdict(text, subject).status !== "incorrect" || isKanaReading(text);

/** Kana once the typing is off it — 。せい is せい with a full stop in front. */
const isKanaReading = (text) => {
  const trimmed = trimReadingPunctuation(text);
  return Boolean(trimmed) && wanakana.isKana(trimmed);
};
...
export function splitAnswer(raw, subject, needsReading) {
  ...
  if (isKanaReading(text)) return { meaning: null, reading: text };
...
function meaningOnly(text, subject) {
  if (!text) return null;
  const forms = meaningForms(text);
  // Right first, then close: a typo with a reading after it — "alcholol. shu"
  // — was being charged twice, once for the slip and once for the tail it
  // stopped the parse from shedding, so it never even looked near.
  return (
    forms.find((form) => isMeaningCorrect(form, subject)) ??
    forms.find((form) => isNearMiss(form, subject)) ??
    text
  );
}

/**
 * The readings of a meaning-only reply worth grading: the whole thing, and
 * the whole thing minus a volunteered reading. Nothing here accepts anything
 * on its own — the caller still has to find one of these actually right.
 */
function meaningForms(text) {
  const forms = [text];

  const parts = text.split(EXPLICIT_SEPARATORS).filter(Boolean);
  if (parts.length === 2) forms.push(parts[0]);

  // Nothing between the halves but a space, which is how most of them arrive:
  // 少 was answered "few shou" and marked wrong, while "few. shou" and
  // "few, shou" both passed. Only the last word comes off, so a meaning that
  // is itself several words ("rib cage") is still tried whole first.
  const words = text.split(/\s+/);
  if (words.length > 1) forms.push(words.slice(0, -1).join(" "));
