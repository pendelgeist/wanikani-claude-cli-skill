...
/** WaniKani's own name for each kind of subject. */
...
/**
 * Which kind of thing is being asked, in one word and in brackets. Absent
 * only if the item somehow arrived without a type.
 */
function typeLabel(item) {
  if (!item.subjectType) return "";
  return ` (${SUBJECT_TYPE_LABELS[item.subjectType] ?? item.subjectType})`;
}

/**
 * The question as it should appear: a number, the characters, and which kind
 * of subject it is. No gloss, no "meaning & reading?" tail — naming the item
 * is naming the answer. Returns null when there's neither a glyph nor an
 * image to show, which is the one case the caller has to handle rather than
 * prompt.
 *
 * The type is part of the question, not a hint, because a glyph on its own is
 * sometimes not a question at all. 末 came up twice in one sitting fifteen
 * minutes apart — once as the vocabulary word (すえ) and once as the kanji
 * (まつ) — and both times the prompt read `末` and nothing else. The user gave
 * each one the other's reading and lost both, and there was nothing on screen
 * either time to tell them which was being asked. WaniKani never shows a bare
 * glyph for this reason: its review screen colours the banner by type and
 * labels every question "Radical Name", "Kanji Reading", "Vocabulary Reading".
 * Withholding that isn't fidelity to the website, it's a missing half of the
 * question — and on a radical it also says that no reading is wanted.
 *
 ...
 * prompt is still a fragment, on purpose: a label is not a question, and what
 * stops the message being finished by hand is that there is no message — see
 * SKILL.md on `grade`.
 */
export function promptFor(item, position) {
  const number = position == null ? "" : `${position}. `;
  if (item.characters) return `${number}${item.characters}${typeLabel(item)}`;
  ...
  // is the radical's name, which is why it never appears in a prompt. The
  // label goes after the whitespace that ends the URL, so link detection
  // still stops where the URL does.
  if (item.characterImageUrl) return `${number}${item.characterImageUrl}${typeLabel(item)}`;
  return null;
}
...
export function criticalNote({ total, shown, under }) {
  const held =
    total > shown ? ` The worst ${shown} of ${total} — \`critical-condition --limit ${total}\` for all of them.` : "";
...
/**
 * Said under a correction when the answer was wrong by a hair, naming the
 * meaning it came within a hair of.
 *
 ...
 *
 * The offer names the word and both halves of the call it asks for, because
 * the first sitting to reach for it reached for it on the wrong item: 伝える
 * answered "to transfer" was flagged (three edits from "To Transmit", one past
 * the tolerance) and forgiven as a typo. It isn't one — it's a different word
 * — and forgiving it credited a level WaniKani wouldn't have given, which is
 * the one way this tool can put someone's record out of step with the site it
 * submits to. A flag is not a verdict; the words next to each other are what
 * the judgment is made on.
 */
export const nearMissLine = (meaning) =>
  `(close to "${meaning}" — a typo on that word is \`answer --forgive meaning\`; a different word isn't)`;

export const ASK_READING = "Reading?";

/**
 * The end of a batch, once the summary has been printed: the turn, handed
 * back, in the same words every time.
 *
 * Everything else at this beat is already finished text — what went in, what
 * moved, what's left — and the one thing that wasn't was the sentence that
 * gives the user the floor. Across eight consecutive batches of one sitting
 * that sentence was written eight different ways, four of them asking whether
 * to *stop* ("Stop?", "Stop wanikani?", "Done?", "Done for today?") of
 * someone who had said "continue" or "next" every single time. The same
 * sitting closed on "(61% accuracy)" — arithmetic on the summary line, and
 * not even the right name for it: `perfect` counts items that went in with
 * nothing wrong on either half, which is not what WaniKani calls accuracy.
 *
 * So it is printed rather than described, like `ANSWER_CONVENTION` and
 * `nearMissLine` before it. It is deliberately not a question: there is
 * nothing to decide, the queue isn't going anywhere, and an invitation to
 * quit every ten items is its own kind of nudge. Only when reviews remain —
 * there is no next batch to offer otherwise.
 */
export const CARRY_ON = "Next batch whenever you're ready.";

/**
 * Said when a fetch mid-sitting picked up reviews the sitting hadn't seen.
 *
 * The count of what's left is the one figure a driver will otherwise invent
 * (see `batchSummaryLine`), and it is also the one that can move the wrong
 * way: one sitting ended a batch on "17 left", the user stepped away for an
 * hour, and the next batch ended on "20 left". Both were right — thirteen
 * reviews had unlocked in between — but nothing on screen said so, and an
 * unexplained number going backwards is how a working tool gets called
 * broken. It says it once, where it happens.
 */
export const newlyDueLine = (count) =>
  `${count} more ${count === 1 ? "review has" : "reviews have"} come due since this sitting started, ` +
  "so what's left has gone up rather than down.";
 ...
 */
/**
 * ...and how to skip one in the middle, which is the half that was missing.
 *
 * "Answer as few as you like" is true of the *end* of the list and says
 * nothing about a gap in it, so an item somebody didn't know got left out
 * rather than blanked — and every answer after it was graded against the
 * question before its own. The empty slot was always the mechanic;
 * `splitReplies` has kept interior blanks open from the start. It just wasn't
 * written down anywhere the person answering would see it.
 */
export const RAPID_CONVENTION =
  'All in one message, in order, separated by "|" — meaning and reading together on each, ' +
  'e.g. "fur, ke | side, yoko". Answer as few as you like; the rest keep, and an empty slot ' +
  '("a | | c") skips one in the middle.';
...
/**
 * What to say when the reply doesn't line up with the questions — a skipped
 * item in the middle rather than a list that stopped short.
 *
 * It asks for the batch again rather than working out the shift on the user's
 * behalf. Re-pairing nine answers onto ten questions and grading the result
 * would be guessing at what somebody meant, on a record that submits to
 * WaniKani; asking is one retype, and the list gets printed underneath so
 * there is nothing to scroll back for.
 *
 * Filling every slot is also the way past a false alarm: a full-length list
 * has nothing to shift, so it is graded exactly as typed. That matters more
 * than the wording — without it, somebody who really did mean an odd-looking
 * reply would have no way to get it recorded.
 */
export function misalignedLine({ open, replies, answerIndex, fitsIndex }) {
  const numbers = open.slice(answerIndex, fitsIndex).map((item) => item.position);
  const skipped =
    numbers.length === 1
      ? `Item ${numbers[0]}`
      : `Items ${numbers.slice(0, -1).join(", ")} and ${numbers.at(-1)}`;

  return (
    `${replies.filter(Boolean).length} answers for ${open.length} open items, and they don't line up with ` +
    `the questions — nothing graded, nothing lost. ${skipped} looks passed over rather than left ` +
    "unanswered, which would put every answer after it against the question before its own. Send the " +
    'batch again with a slot for every item — an empty one ("a | | c") for any being skipped, which ' +
    "leaves it open and unrecorded:"
  );
}
 ...
 * than padded — no "0 burned", no session total on the first batch. What's
 * left is not one of those: "none left" says something, and is the one figure
 * a driver will otherwise invent.
 *
 * `remainingNewlyDue` says that what's left came due while the sitting ran —
 * see `countRemainingReviews`, which is the only thing that can know it.
 */
export function batchSummaryLine({
  ...
  // Zero is the count that most needs saying. It used to fall out of the line
  // as falsy, and the batch that cleared the queue ended on "8 done, 7
  // perfect · 6 leveled up, 1 slipped back" — no word on what was left. The
  // driver filled the gap in prose with "All reviews cleared. Queue reset.",
  // and `summary` one turn later said sixteen reviews were waiting. A number
  // that is always there is a number nobody has to guess at.
  if (remaining !== null && remaining !== undefined) {
    if (remaining === 0) segments.push("none left");
    // Why it went up. Without the clause the number is simply wrong-looking:
    // "7 left" at the end of one batch and "31 left" seven items later, both
    // true, with thirty-one reviews having come due in between and nothing
    // saying so. The driver resolved the contradiction the way a vacuum where
    // a number belongs is always resolved — in prose, and wrongly: "all
    // reviews cleared from earlier batches, session done" went out directly
    // under the thirty-one.
    else segments.push(`${remaining} left${remainingNewlyDue ? " — all of them come due since the last fetch" : ""}`);
  }
