...
import { ANSWER_CONVENTION, DRIVER_NOTE, ASK_READING, newlyDueLine } from "../present.js";
...
export async function askCommand(client, { limit = DEFAULT_BATCH } = {}) {
  ...
  const shown = await showWaiting(client);
  ...
  const notes = [newlyDue > 0 && newlyDueLine(newlyDue), startOfSitting && ANSWER_CONVENTION].filter(Boolean);
