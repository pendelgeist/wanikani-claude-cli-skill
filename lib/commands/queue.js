...
import { loadQueueOrder, clearGradesFor } from "../queueOrder.js";
...
export async function queueCommand(client, { limit, answers = false } = {}) {
  ...
  // than let it submit later as if it were this answer. Writing also keeps the
  // sitting alive, the same as grading in it does.
  await clearGradesFor(queue.map((item) => item.assignmentId));
