...
import { readingTypeLabel, requiresReading, unacceptedReadings, wantedReadingType } from "./grading.js";
 ...
 */
export function promptFor(item, position, { question = true } = {}) {
  const number = position == null ? "" : `${position}. `;
  const tail = question ? questionTail(item) : "";
  if (item.characters) return `${number}${item.characters}${tail}`;
  ...
  // is the radical's name, which is why it never appears in a prompt. The tail
  // goes after it: a space ends a URL, so the link stays clickable.
  if (item.characterImageUrl) return `${number}${item.characterImageUrl}${tail}`;
  return null;
}

/**
 * The question the prompt is asking, in one fixed wording.
 *
 * The tail used to be left off on the grounds that `convention` had said it
 * once and the item number was all the framing a prompt needed. What arrived
 * instead, session after session, was a tail invented on the spot: `— meaning
 * & reading?`, then `— meaning + reading?`, then `— meaning and reading?` on
 * all twenty prompts of one sitting, and finally `—meaning and reading?` sent
 * *without* the prompt in front of it — thirty consecutive messages asking
 * about an item none of them named, the glyph left behind in the tool output.
 *
 * Four spellings in is enough evidence: the tail is going to be there. Issued
 * from here it can't drift in wording, and it can't come away from the item it
 * belongs to, because it is the same string.
 */
function questionTail(item) {
  const needsReading = item.needsReading ?? requiresReading(item.subjectType);
  return needsReading ? " — meaning & reading?" : " — meaning?";
}
 ...
 * because an instruction to say something once is followed once and then
 * improvised on. Saying it here no longer has to keep the `— meaning &
 * reading?` tail off every prompt — `promptFor` issues that tail now, for the
 * reasons in its own note — so this covers what a tail can't: "more", and
...
export function promptListFor(items, { convention = true } = {}) {
  const lines = items.map((item) => {
    // No per-line tail here: ten copies of "— meaning & reading?" down the
    // left of a batch is the noise the tail was originally left off to avoid,
    // and `RAPID_CONVENTION` under the list asks it once for all of them.
    const prompt = promptFor(item, item.position, { question: false });
