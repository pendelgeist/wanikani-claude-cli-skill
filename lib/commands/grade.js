...
import { alreadyAnsweredLine, correctionsFor, readingNudgeFor, ASK_READING, NEAR_MISS } from "../present.js";
...
function decide({ given, data, needsReading, subject }) {
  ...
  if (meaning === "incorrect") {
    ...
    return {
      ...
      // never once been reached for from the skill file alone.
      nearMiss: isNearMiss(given.meaning, data),
      open: false,
    };
  }
...
export function verdictLines(verdict, { label = "", closesBatch = false } = {}) {
  ...
  // whose go it is next.
  if (verdict.nearMiss) lines.push(NEAR_MISS);
