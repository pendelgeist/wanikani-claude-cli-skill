...
function meaningOnly(text, subject) {
  ...
  // "few, shou" both passed — the same answer, lost to the punctuation the
  // user happened not to use. Only the last word comes off, so a meaning that
  // is itself several words ("rib cage") is still tried whole first, and a
  // wrong leading answer stays wrong.
  const words = text.split(/\s+/);
  if (words.length > 1) {
    const withoutLast = words.slice(0, -1).join(" ");
    if (isMeaningCorrect(withoutLast, subject)) return withoutLast;
  }
  return text;
}
