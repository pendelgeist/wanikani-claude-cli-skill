...
import { loadQueueOrder, loadSitting, beginBatch, takeNewlyDue } from "../queueOrder.js";
import {
  promptFor,
  correctionsFor,
  otherReadingHelp,
...
export async function serveNewBatch(client, { limit, restart = false } = {}) {
  ...
  // Read *after* the fetch, and off the record rather than off `order`. A
  // sitting resumed after a break has aged out of `loadQueueOrder` — that is
  // what sends it here for a fresh list — but the fetch carries its totals
  // onto the new one, so it is not the start of anything. Asking beforehand
  // handed the opening how-to to someone forty items in, under the item they
  // were being asked next.
  const startOfSitting = ((await loadSitting())?.totals?.submitted ?? 0) === 0;
  // Read once and cleared, so it lands on the batch that was fetched with it
  // rather than on every batch after.
  const newlyDue = await takeNewlyDue();
  ...
  return { queue, startOfSitting, newlyDue };
}

export async function queueCommand(client, { limit, answers = false, restart = false } = {}) {
  const { queue, startOfSitting, newlyDue } = await serveNewBatch(client, { limit, restart });
  ...
  const payload = queue.map(({ subject, primaryMeaning, primaryReading, ...item }, index) => ({
    ...
    ...(index === 0 && newlyDue > 0 ? { newlyDue: newlyDueLine(newlyDue) } : {}),
    ...(answers
      ? {
