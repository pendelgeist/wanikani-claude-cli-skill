...
/**
 * The accepted readings, named with their type as well as their kana, because
 * "which of this kanji's readings does WaniKani actually want?" is the
 * question a missed kanji reading leaves behind and the answer is in the
 * data. Vocabulary readings carry no type and come back as bare kana.
 *
 * Grouped: one label per type rather than one per reading. 軽 accepts
 * かる and かろ, and its correction read "reading is かる (kun'yomi) / かろ
 * (kun'yomi)" — the same thing said twice on the one line whose whole job is
 * to be read carefully.
 */
const describeAcceptedReadings = (readings) =>
  groupReadingsByType((readings ?? []).filter((entry) => entry.accepted_answer));
...
export function correctionsFor(item) {
  ...
  const reading = readings ? `reading is ${readings}` : null;
...
/**
 * "おや / した (kun'yomi)" — one label per type, not one per reading. Keyed by
 * the label rather than the raw type so that an untyped reading (a
 * vocabulary word's) groups under "no label" and comes back as bare kana,
 * which is what it has always printed as.
 */
function groupReadingsByType(readings) {
  ...
  for (const entry of readings) {
    ...
    byLabel.get(label).push(entry.reading);
  }
  return [...byLabel]
    .map(([label, kana]) => (label ? `${kana.join(" / ")} (${label})` : kana.join(" / ")))
    .join(", ");
}
...
export function explainBlock(item) {
  ...
  if (readings.length) {
    ...
    lines.push(`Reading: ${groupReadingsByType(readings)}${also}`);
  }
...
/**
 * The sitting's score, as a count and as the percentage that otherwise gets
 * worked out by hand.
 *
 * The percentage is here because three sittings in a row closed on one that
 * nobody asked for — "47 total (77% perfect)", "94 total (72% perfect)" —
 * computed in prose from the two numbers already on the line. They happened
 * to be right. The same arithmetic done the same way has also produced "60
 * reviewed, 46 perfect" against a real 70 and 53, and every miscount in
 * FAILURES.md started as a figure somebody worked out rather than read. A
 * number that is on the screen is a number nobody derives.
 *
 * Batch-level percentages stay off: a share of ten is noise, and it isn't
 * what was being computed.
 */
function perfectShare(submitted, perfect) {
  if (perfect === null || perfect === undefined) return "";
  if (submitted <= 0) return `, ${perfect} perfect`;
  return `, ${perfect} perfect (${Math.round((perfect / submitted) * 100)}%)`;
}
...
export function batchSummaryLine({
  ...
  if (sessionSubmitted !== null && sessionSubmitted > submitted) {
    segments.push(`${sessionSubmitted} done this sitting${perfectShare(sessionSubmitted, sessionPerfect)}`);
  }
