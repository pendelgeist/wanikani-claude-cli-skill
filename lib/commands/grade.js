...
import { loadSitting, recordGrade, forgiveGrade, awaitingFor } from "../queueOrder.js";
...
export async function gradeCommand(client, { json = false, ...options }) {
  reportVerdict(await gradeAnswer(client, options), json);
}
...
export async function gradeAnswer(client, { subjectId, answer, meaning, reading, forgive }) {
  ...
  if (forgive) {
    ...
    return { subjectId, assignmentId, forgave: forgive, recorded, warning };
  }
  ...
  const recorded = assignmentId
    ...
    : null;
  return { ...verdict, recorded, warning };
}
 ...
 */
export function verdictLines(verdict, { label = "" } = {}) {
  const lines = [];
  ...
  else if (verdict.say) lines.push("(recorded — next item)");

  if (verdict.warning) lines.push(`! ${verdict.warning}`);
  return lines;
}

function reportVerdict(verdict, json) {
  if (json) {
