...
/**
 * A URL a terminal will linkify all the way to its end. Terminal link
 * detection runs on an ASCII URL character set, so a link with a glyph in it
 * — `https://jisho.org/search/追%20%23kanji`, `https://www.wanikani.com/kanji/親`
 * — is ctrl-clickable only as far as the glyph, and the rest has to be
 * copied out by hand into a browser. Percent-encoding says exactly the same
 * thing to the server (it is what a browser sends for the raw form anyway)
 * in characters every linkifier follows to the end.
 */
export function clickableUrl(url) {
  if (!url) return url;
  return url.replace(/[^\x20-\x7E]+/gu, (run) => encodeURIComponent(run));
}
