...
import { alreadyAnsweredLine, correctionsFor, nearMissLine, readingNudgeFor, ASK_READING } from "../present.js";
import { nextInBatch, BATCH_DONE } from "./prompts.js";
import { isRecentMiss } from "../misses.js";
import { isCriticalItem } from "../critical.js";
...
function decide({ given, data, needsReading, subject }) {
  ...
  if (meaning === "incorrect") {
    ...
    return {
      ...
      // Wrong by a hair is worth saying so, since the override for it has
      // never once been reached for from the skill file alone. The field is
      // the meaning it was close to rather than a flag: the call it invites is
      // a comparison, and a comparison needs both words.
      nearMiss: nearMissMeaning(given.meaning, data),
      open: false,
    };
  }
 ...
 */
/**
 * Whether an answer that belongs to no batch is a drill rather than a
 * mistake. Two lists qualify, and neither of them submits: what this tool
 * watched you get wrong (`drill`) and what WaniKani's own records have you
 * failing (`critical`). Both are read from disk, so a session that fetched
 * either one is still recognised by a session that didn't.
 */
const isDrilled = async (subjectId) => (await isRecentMiss(subjectId)) || (await isCriticalItem(subjectId));
...
async function placeInSitting(subjectId) {
  ...
  if (!order) {
    if (await isDrilled(subjectId)) return { assignmentId: null, warning: null, drill: true };
    return {
      assignmentId: null,
      warning:
...
export function verdictLines(verdict, { label = "", closesBatch = false } = {}) {
  ...
  // Above the turn markers, because it's about the verdict rather than about
  // whose go it is next.
  if (verdict.nearMiss) lines.push(nearMissLine(verdict.nearMiss));
  ...
  // the line under this one says what actually comes next.
    ...
    // A drill records nothing, and the line below says so outright — so this
    // one must not claim otherwise directly above it. A missed drill item
    // printed "(recorded — next item)" and "(drill — nothing recorded,
    // nothing submitted)" back to back, which is the contradiction a driver
    // is then left to resolve in prose, and prose is where this goes wrong.
    // The marker still has to be here: whose turn it is next is the question
    // the correction leaves open, and "no retry" is rule 5.
    if (verdict.drill) lines.push("(next item)");
    else lines.push(closesBatch ? "(recorded)" : "(recorded — next item)");
  }
