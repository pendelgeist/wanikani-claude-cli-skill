...
async function pendingAssignments(client) {
  ...
  await saveQueueOrder(items, {
    ...
    newlyDue: arrivedSince(items, previous),
  });
  return items;
}

/**
 * How many of the freshly fetched assignments are ones the sitting couldn't
 * have known about — which is a different question depending on what it has
 * to compare against.
 *
 * While it still holds a list, that list is the baseline and the answer is a
 * set difference: the items it had are the items it knew were due, and
 * anything else arrived. Once the list is empty the set says nothing — every
 * item looks new — so the baseline is the number the sitting last *reported*,
 * which is what the API told it was still out there when its last batch
 * submitted.
 *
 * The empty-list case is not an edge: it is the shape every sitting is left
 * in by its final submit, and so the shape every resumed sitting starts from.
 * One opened on "47 more reviews have come due since this sitting started"
 * with forty-seven due, sixteen of them actually new and the note printed
 * under the sitting's very first question.
 *
 * With no baseline at all — a first fetch, or a record written before any
 * count was kept — the answer is zero, because "nothing to say" is the honest
 * reading of "nothing to compare against".
 */
function arrivedSince(items, previous) {
  if (!previous) return 0;
  const held = previous.items ?? [];
  if (held.length > 0) {
    const seen = new Set(held.map((item) => item.assignmentId));
    return items.filter((item) => !seen.has(item.assignmentId)).length;
  }
  const reported = previous.lastRemaining;
  if (reported === null || reported === undefined) return 0;
  // Never negative: a queue that shrank (reviews done on the website, say) is
  // not a queue that grew, and this line only ever explains growth.
  return Math.max(0, items.length - reported);
}
  ...
  try {
    ...
    const counted =
      order && order.items.length > 0
        ? { remaining: order.items.length, unfetched: false }
        : {
            remaining: (await client.getAssignments({ immediately_available_for_review: true })).length,
            unfetched: Boolean(order),
          };
    // Filed as it's reported, because a count on screen is the only baseline
    // the next fetch will have once the list this one came from is empty.
    await rememberRemaining(counted.remaining);
    return counted;
  } catch {
