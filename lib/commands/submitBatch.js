 ...
 *
 * Prints the end-of-batch line — what went in, what moved, what's left — and
 * nothing else, because that line is the whole of what the sitting needs and
 * anything longer gets folded off the screen. `--json` gives the full
 * `{ summaryLine, results, batch, remaining }` payload instead, for debugging
 * this CLI.
 *
...
export async function submitBatchCommand(client, { ignoredStdin = false, json = false } = {}) {
  ...
  // The line, and nothing else. This used to print the whole payload, and the
  // whole payload is a hundred and fifty lines for a batch of ten — of which
  // Claude Code shows the first three and folds the rest behind "ctrl+o to
  // expand". So the end of every batch arrived on screen as `{`,
  // `"summaryLine": …`, `"results": [`, and the user had to expand a JSON
  // dump to read a sentence that was already written for them. The payload is
  // still there under --json for anything that wants the per-item SRS
  // movement; nothing in a session does, because `ask` submits in-process and
  // the highlights are in the line.
  console.log(payload.summaryLine);
  // Not only on stderr, for the same reason it's in the payload: the sitting
  // that piped a hand-written list into all six of its submits never saw the
  // warning, because stderr is not what gets read back.
  if (payload.ignoredStdin) console.log(`! ${payload.ignoredStdin}`);
}
...
export async function submitBatch(client, { ignoredStdin = false } = {}) {
  ...
  if (batchItems.length === 0) {
    ...
    return {
      ...
      remaining: (await countRemainingReviews(client)).remaining,
  ...
  const { remaining, unfetched } = await countRemainingReviews(client);
  ...
  return {
    summaryLine: batchSummaryLine({
      ...
      remaining,
      // Only when there's something to explain: the count has gone up because
      // the sitting's list ran out and the API had more than it started with.
      remainingNewlyDue: unfetched && remaining > 0,
