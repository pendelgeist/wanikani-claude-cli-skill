...
/**
 * Every kana this romaji could reasonably mean. The alternate parse is only
 * ever *accepted* when it turns out to be one of the item's readings, so this
 * can't promote a wrong answer — it can only stop an apostrophe deciding
 * whether someone knew the word.
 */
export function readingCandidates(input) {
  const text = normalizeAmbiguousRomaji((input ?? "").trim());
  const asTyped = wanakana.toKana(text);
  const separated = wanakana.toKana(text.replace(AMBIGUOUS_N, "n'"));
  return separated === asTyped ? [asTyped] : [asTyped, separated];
}
...
export function isReadingCorrect(input, subject) {
  const candidates = readingCandidates(input).filter(Boolean);
...
export function readingVerdict(input, subject) {
  ...
  const candidates = readingCandidates(input).filter(Boolean);
  const other = unacceptedReadings(subject).find((r) => candidates.includes(r.reading));
...
export function splitAnswer(raw, subject, needsReading) {
  const text = (raw ?? "").trim();
  if (!needsReading) return { meaning: text || null, reading: null };
