...
export class WaniKaniClient {
  constructor(token) {
    if (!token) {
      throw new WaniKaniError(
        "No API token found. Copy .env.example to .env and paste your token in, " +
          "or export WANIKANI_API_TOKEN (wanikani.com → Settings → API Tokens).",
      );
    }
    this.token = token;
  }

  async request(path, { method = "GET", body, permission } = {}) {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          // The call knows which box it needed, so the error says which box to
          // tick rather than listing every permission the CLI might ever want.
          message += permission
            ? ` — this call needs the token's ${permission} permission (wanikani.com → Settings → API Tokens)`
            : " — check the token's permissions (wanikani.com → Settings → API Tokens)";
        }
  ...
  /**
   * The running correct/incorrect tally WaniKani keeps per item. Filterable
   * server-side — `percentages_less_than` is what the critical-condition list
   * is built out of.
   */
  getReviewStatistics(params = {}) {
    const qs = new URLSearchParams(params).toString();
    return this.getCollection(`/review_statistics${qs ? `?${qs}` : ""}`);
  }
