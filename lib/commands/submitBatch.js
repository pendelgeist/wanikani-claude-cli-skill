 ...
 *
 * `items` is optional: `grade` has been keeping each item's wrong-answer
 * counts as it went, so passing nothing submits exactly what was graded this
 * batch. An explicit list still wins where it's given — that's the override
 * path, for a verdict a human overruled after the fact.
 */
export async function submitBatchCommand(client, items) {
  const grades = await loadGrades();
  const batchItems =
    items ??
    Object.entries(grades).map(([assignmentId, counts]) => ({ assignmentId: Number(assignmentId), ...counts }));
  // An empty record where a whole batch should be is worth saying out loud:
  // "0 done, 0 perfect" reads like a submitted batch of nothing. The usual
  // cause is a sitting that aged out (the queue order has a 30-minute life),
  // which loses the counts but not the items — they're still due.
  if (!items && batchItems.length === 0) {
    console.log(
      JSON.stringify(
        {
          summaryLine:
            "Nothing submitted — no grades on record for this batch. " +
            "If the sitting aged out, those items are still due and can be re-answered.",
          results: [],
  ...
  for (const item of batchItems) {
    const recorded = grades[item.assignmentId] ?? {};
    const wrongMeaning = item.wrongMeaning ?? recorded.wrongMeaning ?? 0;
    const wrongReading = item.wrongReading ?? recorded.wrongReading ?? 0;
    try {
