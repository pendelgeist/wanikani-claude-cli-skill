...
async function pendingAssignments(client) {
  ...
  // A live sitting that has simply run out of fetched items is still the same
  // sitting, so its running totals carry over the refetch. They didn't: a
  // sitting that reported "79 done this sitting, 56 perfect" cleared its list,
  // pulled the reviews that had come due since, and reported the very next
  // batch as ten — the first seventy-nine gone from a count the user was
  // watching. `null` is the other case, an expired or absent sitting, and that
  // one really does start from zero.
  await saveQueueOrder(items, order ? carriedOver(order) : {});
  return items;
}
