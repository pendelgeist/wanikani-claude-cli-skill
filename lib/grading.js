 ...
 */
const acceptedMeanings = (subject) =>
  [
...
export function isNearMiss(input, subject) {
  ...
  if (!normalizedInput || isMeaningCorrect(input, subject)) return false;
  if (blacklistedMeanings(subject).includes(normalizedInput)) return false;

  return acceptedMeanings(subject).some(
    (entry) => editDistance(normalizedInput, entry) <= typoTolerance(entry) + 1,
  );
}
